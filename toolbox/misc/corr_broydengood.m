function [prob cont corr] = corr_broydengood(prob, cont, func)
...
corr.x     = x0;
[prob chart corr.f corr.Jb] = corr.FDF(prob, chart, corr.x);
% if ~isfield(corr, 'JInv') || size(corr.JInv,1)~=numel(x0)% MINGWU'S CHANGE-tackle remesh event
%       % bug: move JInv to chart data
%       % bug: catch remesh event
%       % bug: sparse updates (of factorisation?)
%       %   corr.JInv  = eye(numel(x0));
%       % use finite difference method to do it
%       [prob chart corr.f corr.J] = corr.FDF(prob, chart, corr.x); % MINGWU'S CHANGE-calculate by numerical difference
%       corr.JInv  = inv(corr.J);
% else
%   [prob chart corr.f] = corr.F(prob, chart, corr.x);  
% end
% [prob chart corr.f] = corr.F(prob, chart, corr.x);  corr.Jb = eye(numel(x0));
corr.It    = 0;
corr.SubIt = 0;
corr.ftm   = 0;
